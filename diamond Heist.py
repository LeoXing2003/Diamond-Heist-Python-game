...
def background(): #Draws the background
    ...
    #screen.create_rectangle(520,390,550,530)

    
def endgame():#when called this function will end the game
    global protip,qpressed
    cover2 = screen.create_rectangle(0,0,1000,800,fill = "white") #Completely covers the screen
    ...
    if qpressed == True: #Quits the game if q is pressed
        root.destroy()
...
def update(): #Constant updates the position of the character
    #Checks for any collision between the player and any walls
...
def light(): #Creates the light beams
    ...
    radians = i*pi/180 #Determines the circumference that the light will be moving in
    ...
    #Variable i will increase by 1 after each frame which will update the light beam
...
def detectCollision(): #Responsible for checking collisions between the player and the light beams
    ...
    if (guardx1 <= xposition <= xlight1 or xlight1 <= xposition <= guardx1) or (guardx2 <= xposition <= xlight2 or xlight2 <= xposition <= guardx2) or (guardx3 <= xposition <= xlight3 or xlight3 <= xposition <= guardx3):
        #If light is not vertically-aligned
        if (xlight1 - guardx1) != 0 and (xlight2 - guardx2) != 0 and (xlight3 - guardx3) != 0:
            ...
            #y-intercept calculations using the equation y = mx + b (b = y-mx)
            yint1 = ylight1 - (slope1 * xlight1)
            yint2 = ylight2 - (slope2 * xlight2)
            yint3 = ylight3 - (slope3 * xlight3)

            #Substituting player's x position into three equations
            ...
            if (guardx1 <= xposition <= xlight1 or xlight1 <= xposition <= guardx1) and abs(yposition - y1) <= 10:
                #If the conditions mentioned above are true, the game will stop running
                run = False 
                print("Hit1")
        ...
        else: #Light is vertically-aligned, meaning that if the y coordinates are the same
